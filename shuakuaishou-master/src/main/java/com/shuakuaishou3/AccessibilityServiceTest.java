...
public class AccessibilityServiceTest extends AccessibilityService {
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        Log.d(TAG, "onAccessibilityEvent 进入成功.");
                    ...
                    public void run() {
                        try{
                            while(true) {
                             ...
                             //           AccessibilityServiceTest.this.getRootInActiveWindow();
                                //List<AccessibilityNodeInfo> listInfo =
                            //            rootInfo
                                //            .findAccessibilityNodeInfosByText("点击重播");
                             //   if (listInfo.size() ==1){
        ...
        //进入快看点,或者今日头条
        if (event.getPackageName().equals("com.yuncheapp.android.pearl") ||event.getPackageName().equals("com.ss.android.article.lite") ){
            Log.d(TAG, "current view in  快看点 或今日头条interface.");
            final AccessibilityNodeInfo nodeInfo = event.getSource();
            if (nodeInfo != null) {
                Log.d(TAG, "nodeinfo:" + nodeInfo.getClassName());
            }
                        ...
                        while(true) {
                            ...
                            List<AccessibilityNodeInfo> listInfo =
                                        ...
                                        .findAccessibilityNodeInfosByViewId(
                                                "com.yuncheapp.android.pearl:id/title");
                               if (listInfo.size()>= 1) {
                                   Log.d(TAG, "找到相关推荐");
                                   listInfo.get(0).performAction(AccessibilityNodeInfo.ACTION_CLICK);
                               }else{
